...
function notification_new_post($post_id=0) {
	...
	$all_users = notification_select_all_users($topic_id);
...
function notification_select_all_users($topic_id) {
	global $bbdb, $bb_current_user;
	
	$query = "SELECT ID, user_email FROM wp_users " . 
		  "WHERE user_status=0 AND ".
		  "ID in (SELECT poster_id FROM bb_posts ". 
		  	   "WHERE topic_id = $topic_id AND post_status=0 AND poster_id <> $bb_current_user->ID)";
	$all_users = $bbdb->get_results($query);
...
function notification_profile() {
	...
	echo "notification profile";
	if ( bb_is_user_logged_in() ) :
		echo "notification profile inside";
		$checked = "";
