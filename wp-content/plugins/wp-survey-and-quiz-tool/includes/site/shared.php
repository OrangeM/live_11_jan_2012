...
function wpsqt_site_shared_email(){
	...
	if ( empty($emailTemplate) ){
		...
		if ( preg_match_all('/%USERMETA_(.*)%/isU',$emailMessage,$matches) ){			
			foreach( $matches[1] as $match ){				
				$userMeta = get_user_meta($current_user->ID, $match, true);
				
				//print_r($userMeta); exit;
				
				if($userMeta != '')
					$emailMessage = str_ireplace('%USERMETA_'.$match.'%', $userMeta, $emailMessage);
			}			
		}
	...
	$headers = 'From: AdviserVoice <'.$fromEmail.'>' . "\r\n";

	foreach( $emailList  as $emailAddress ){		
		add_filter('wp_mail_content_type', create_function('', 'return "text/html"; '));
		wp_mail($emailAddress,'CPD Notification', stripslashes($emailMessage), $headers);			
	}
	
	$headers = array(
			'From' 		=> 'From: AdviserVoice <'.$fromEmail.'>',
	...
	add_filter('wp_mail_content_type', create_function('', 'return "text/html"; '));	
	wp_mail($objUser->user_email,'CPD Notification',stripslashes($emailMessage),$headers);		
}
