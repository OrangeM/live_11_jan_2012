<?php global $blog_id; ?>
<?php $title_max_char=50;?>
<script language="javascript" type="text/javascript">

	jQuery(function()
	{
		var topic = jQuery('#quiz_name')[0];
		var titleChars = jQuery('#title-chars-left')[0];
		limitText(topic, titleChars);
		jQuery('#quiz_name').keydown(function(){limitText(topic, titleChars);});
		jQuery('#quiz_name').keypress(function(){limitText(topic, titleChars);});
		jQuery('#quiz_name').keyup(function(){limitText(topic, titleChars);});
		...
		<table class="form-table" id="question_form">
			<tbody>
				<tr>
					...
					<td valign="top"><input id="quiz_name" maxlength="<?php echo $title_max_char ?>" size="50" name="quiz_name" value="<?php if ( isset($quizDetails["name"]) ) { echo stripslashes($quizDetails["name"]); } ?>" /></td>
					<td>What you would like the quiz to be called. Name should be not more than <?php echo $title_max_char ?> characters.<br>
Left: <span id="title-chars-left"><?php echo $title_max_char ?></span></td>
				</tr>
				<tr>
