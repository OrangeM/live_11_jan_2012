...
<div class="wrap">
	...
	<?php foreach ( $result['sections'] as $section ){ ?>
		...
		<?php
			...
			$totalPoints += $section['stats']['correct'] + $section['stats']['incorrect'];
			$currentPoints += $section['stats']['correct'];
			...
			<?php } else { 
				...
				<p><b>Mark</b> <input type="hidden" name="old_mark[<?php echo $questionKey; ?>]" id="old_mark_<?php echo $questionKey; ?>" value="<?php echo (isset($questionArray['mark']) && ctype_digit($questionArray['mark']) ? $questionArray['mark'] : 0 ); ?>" /> <select name="mark[<?php echo $questionKey; ?>]" class="mark" id="current_mark_<?php echo $questionKey; ?>">
					...
					?>
							<option value="<?php echo $i; ?>" <?php   if ( isset($questionArray['mark']) && $questionArray['mark'] == $i ){ ?> selected="yes"<?php }?>><?php echo $i; ?></option>
					<?php } ?>
		...
		<?php } ?>
	<?php } ?>
	<p><font size="+3">Mark: <span id="total_points"><?php echo $currentPoints; ?></span> out of <?php echo $totalPoints; ?> 
	(<?php echo $totalPoints == 0 ? '0' : number_format ($currentPoints * 100 / $totalPoints, 1) ?>%)</font></p>
	<select name="status">
