...
function wp_new_user_notification($user_id, $plaintext_pass = '') {
	...
	@wp_mail(get_option('admin_email'), sprintf(__('[%s] New User Registration', 'sabre'), get_option('blogname')), $message, $headers);	
	...
	if ($sabre_opt['enable_confirm'] == 'user') {
		...
		$message .= get_option('siteurl') . "/wp-login.php?sabre_confirm=" . md5($user_id) . "\r\n";
	}
