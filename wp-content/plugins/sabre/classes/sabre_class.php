...
function login_head() {
	...
	if (isset($_REQUEST['sabre_confirm']) && !empty($_REQUEST['sabre_confirm'])) {
		...
		if ($enable_confirm == 'user') {
			...
			if ((strtotime($result->last_mod) + (int)$period*24*60*60 >= $curdate) && ('to confirm' == $result->status || 'ok' == $result->status) ) {
				...
				$_SESSION['new_user_id'] = $result->user_id;
				wp_redirect( site_url('registration'));
				}
			else {
...
function new_user_confirmation($user_id) {
	...
	@wp_mail(get_option('admin_email'), sprintf(__('[%s] New User Registration Confirmation', 'sabre'), get_option('blogname')), $message, $headers);
}
...
global $wpdb;
	...
	$sabre_opt = $this->get_option('sabre_opt');
	return $wpdb->get_var("SELECT COUNT(*) FROM `" . SABRE_TABLE . "` WHERE `status`= 'ko'");
...
global $wpdb;
	...
	$sabre_opt = $this->get_option('sabre_opt');
	return $wpdb->get_var("SELECT COUNT(*) FROM `" . SABRE_TABLE . "` WHERE `status`= 'ok'");
...
global $wpdb;
	...
	$sabre_opt = $this->get_option('sabre_opt');
	return $wpdb->get_var("SELECT COUNT(*) FROM `" . SABRE_TABLE . "` WHERE `status`= 'to confirm'");
