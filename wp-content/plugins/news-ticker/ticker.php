...
if($phpmaj>=PHPREQ){
  require_once($_SERVER['DOCUMENT_ROOT']."/wp-content/plugins/news-ticker/rss.php");
 }
...
function ticker_js(){    
    if(!is_admin())
    {    
	echo "<script src='http://www.adviservoice.com.au/wp-content/plugins/news-ticker/jcarousel.js' type='text/javascript'></script>";
    }
}
function insert_newsticker(){
echo "<!--debug start_newsticker -->"; 
   ticker_js();
echo "<!--debug start_newsticker -->"; 
  $tickerspeed=get_option('ticker_speed').'050'; //speed in ms, not in seconds
  $tickertimeout=get_option('ticker_timeout'); //speed in seconds
  $tickeranimation=get_option('ticker_anim');
  $tickerheight=get_option('ticker_ht');
  echo "<!--debug insert_newsticker -->";

  $movebystep = 1;
  if($tickeranimation == 'scrollRight')
	$movebystep = -1;
...
.news-ticker{
background:  url(<?php echo  'http://www.adviservoice.com.au/wp-content/plugins/news-ticker/images/news-bg.PNG' ?>) repeat-x top left;
height: <?php echo $tickerheight; ?>px; /*Height should enough to fit largest content's height*/
}

.news-ticker li{
background: url(<?php echo 'http://www.adviservoice.com.au/wp-content/plugins/news-ticker/images/news-separator.PNG' ?>) no-repeat top left;
margin-left: 10px;
}
...
<script type="text/javascript" language="javascript">
	...
	wrap: 'circular',
	auto: <?php echo $tickertimeout;?>,
	animation: <?php echo $tickerspeed;?>,
	scroll: <?php echo $movebystep; ?>
	}); 
 ...
 /* stepcarousel.setup({
	...
	panelbehavior: {speed: <?php echo $tickerspeed;?>, wraparound:true, wrapbehavior:"slide", persist:true},
	defaultbuttons: {enable: false},
	autostep: {enable:true, moveby:<?php echo $movebystep; ?>, pause:<?php echo $tickertimeout;?>},
	contenttype: ['inline'],
	onslide:function(){
		var addAfterLast = false;
		var addBeforeFirst = false;
 		if(<?php echo $movebystep; ?> < 0)
			addBeforeFirst = true;
		else
			addAfterLast = true;
...
function ticker_content(){
 ...
 if($type_opt_val=='recent-comments' && $rss_opt_val=='norss'){
   ...
   case 'norss':
     ...
     break;
   case 'externalwithposts':
	$posts = get_rss_plus_recent_posts();
	break;	
   case 'norss-comments':
     break;
   default:
...
if (!empty($items)) : 
      for($i = 1; $i <= $maxnum; $i++) : 
	$item = $items[i]; ?> 
<li><span class="tickerDate"><?php $pubdate = substr($item['pubdate'], 4, 12); echo $pubdate; ?></span> - <span class="tickerLink"><a href="<?php echo $item['link']; ?>"><?php echo $item['title']; ?></a></span></li>
<?php endfor; 
endif; 
} else if($rss_opt_val=='externalwithposts'){
 foreach ($posts as $post_id => $post){
   ...
   $image_src=$posts[$post_id]['image_src'];
?>
   
<li><?php if($images_opt_val=='checked' && !empty($image_src)) { ?><img src="<?php echo $image_src; ?>" /><?php } ?><div class="ticker-wrapper"><?php if($dates_opt_val=='checked') { ?><span class="tickerDate"><?php echo $date; ?></span> - <?php } ?><span class="tickerTitle"><?php echo $title;?></span><?php if($content_opt_val=='checked') { ?> - <span class="tickerText"><?php echo $excerpt; ?>...</span><?php } ?><a href="<?php echo $link; ?>">Read more</a></div></li>
...
/**
 * Get an array of recent comments combined with external rss feed
 */

function get_rss_plus_recent_posts()
{
	//get rss
   	$namenum = get_option('ext_rss');
     	$maxnum = get_option('ext_rss_num');
	...
	//combine them
	$posts = array();
	foreach ($recent_posts as $post_id => $post) {
		$date_str = $post['post_date'];
	       $date = ticker_parse_date($date_str);
       	$posts[$post_id]['post_date'] = $date;
		$posts[$post_id]['post_human_date'] = ticker_date_to_human_date($date);
		$posts[$post_id]['post_title'] = ticker_html_to_text($post['post_title']);
	       $posts[$post_id]['post_excerpt'] = ticker_html_to_text($post['post_excerpt']);
		$url = get_post_meta($post_id, 'slide_redirect_url', true);
		if($url != '')
			$posts[$post_id]['url'] = get_post_meta($post_id, 'slide_redirect_url', true);
		else
		 	$posts[$post_id]['url'] = $post['url'];
		$data = ticker_recent_imgs($post_id);
		$posts[$post_id]['image_src'] = $data[0]->guid;
//		echo "<!-- image of post".$post_id.":".$posts[$post_id]['image_src']. "-->";
//		echo "<!-- ".print_r($post). "-->";
	}
	for($i = 1; $i <= $maxnum; $i++)
	{
		...
		$item_to_post['post_date'] = $date;
		$item_to_post['post_human_date'] = ticker_date_to_human_date($date);
		$item_to_post['url'] = $item['url'];
		$auto_excerpt_length_opt_val = get_option('ticker_auto_excerpt_length');
		$item_to_post['post_excerpt'] = $item['post_excerpt'];
		
		$output = preg_match_all('/<img.+src=[\'"]([^\'"]+)[\'"].*>/i', $item['atom_content'], $matches);
  		$first_img = $matches [1] [0];
		if(!empty($first_img))
			$item_to_post['image_src'] = $first_img; 
		$item_to_post['post_title'] = $item['post_title'];
		echo '<!-- rss link: '.$item['link'].'-->';
		$posts[]= $item_to_post;
	}
	//re-order items by post date
	usort($posts, "cmp_by_date");
	return $posts;
}
...
function ticker_get_posts($type, $cat_filter, $n, $post_list=null){
	...
	if(THIS_IS_FORUM == 1)
	{
		...
		// TODO: add url for posts in forum
		foreach($posts_fixed as $post_id => $post){
			//     $post['url'] = apply_filters('the_permalink', get_permalink($post_id));
			$date_str = $post['post_date'];
	       	$date = ticker_parse_date($date_str);
			...
			$url_for_post = "http://www.adviservoice.com.au/".$year."/".$month."/".$title."/";
			$posts_fixed[$post_id]['url'] = $url_for_post;
			echo "<!--url: ". $url_for_post."-->";
		}
	}
	else
	{
...
function ticker_recent_imgs($post_id) { 
	...
	{
		global $bbdb;
		return $bbdb->get_results( "SELECT guid FROM wp_posts WHERE post_parent=$post_id AND post_type = 'attachment' ORDER BY 'post_date' ASC LIMIT 0,1" );
	}
	else
	{
		global $wpdb;
		return $wpdb->get_results( "SELECT guid FROM wp_posts WHERE post_parent=$post_id AND post_type = 'attachment' ORDER BY 'post_date' ASC LIMIT 0,1" );	
	}
}
...
<div class="wrap">
  ...
  <form name="form1" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
    ...
    <table class="form-table">
      ...
      <tr valign="top">
        ...
        <td><input disabled="disabled" type="radio" name="<?php echo $type_opt_name; ?>" value='popular' <?php if($type_opt_val=='popular') { echo 'checked'; } ?>>
          Most Popular Posts over the last
          <input disabled="disabled" type="text" name="<?php echo $popular_days_opt_name; ?>" value="<?php echo $popular_days_opt_val; ?>" size="2">
          days (<a href='http://wordpress.org/extend/plugins/stats/'>Wordpress.com Stats Plugin</a> <?php echo $stats_installed_str; ?>)<br/>
          <input disabled="disabled" type="radio" name="<?php echo $type_opt_name; ?>" value='commented' <?php if($type_opt_val=='commented') { echo 'checked'; } ?>>
          Most Commented Posts<br/>
          <input disabled="disabled" type="radio" name="<?php echo $type_opt_name; ?>" value='recent' <?php if($type_opt_val=='recent') { echo 'checked'; } ?>>
          Recent Posts<br/>
          <input disabled="disabled" type="radio" name="<?php echo $type_opt_name; ?>" value='recent-comments' <?php if($type_opt_val=='recent-comments') { echo 'checked'; } ?>>
          Recent Comments<br/>
  ...
  if($phpmaj<PHPREQ){?>
        ...
        <?php }else{ ?>
        <br />
        <input disabled="disabled" type="radio" name="<?php echo $rss_opt_name; ?>" value='norss' <?php if($rss_opt_val=='norss'){echo 'checked';} ?> >
        No overwrite: Use setting above<br/>
        <input disabled="disabled" type="radio" name="<?php echo $rss_opt_name; ?>" value='entries' <?php if($rss_opt_val=='entries'){echo 'checked';} ?>>
        Use contents of Entries RSS feed <br/>
        <input disabled="disabled" type="radio" name="<?php echo $rss_opt_name; ?>" value='comments' <?php if($rss_opt_val=='comments'){echo 'checked';} ?>>
        Use contents of Comments RSS feed<br/>
        <input disabled="disabled" type="radio" name="<?php echo $rss_opt_name; ?>" value='external' <?php if($rss_opt_val=='external'){echo 'checked';} ?>>
        ...
        Number of entries to display: <input type="text" name="<?php echo $ext_rss_num_name; ?>" value="<?php echo $ext_rss_num_val; ?>" size="2" /><br/>
        <input type="radio" name="<?php echo $rss_opt_name; ?>" value='externalwithposts' <?php if($rss_opt_val=='externalwithposts'){echo 'checked';} ?>>
	Use External RSS feed with Recent posts<br/
        </td>
        <?php } ?>
      </tr>
      <tr valign="top">
        ...
        <td> Select the categories to include.  Select one or more categories to restrict post selection to those categories.<br />
          ...
          <select disabled="disabled" style="height: auto;" name="<?php echo $category_filter_opt_name; ?>[]" multiple="multiple">
            <?php 
