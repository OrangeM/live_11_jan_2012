...
function kjgrc_get_comments2 ($requested_comment_type,$caller)
{
	...
	if(is_category() || is_single()){   
		...
		$args = array('child_of' => $cat_ID, 'orderby' => 'name', 'order' => 'ASC');
	...
	while ($comments_number < $requested_number_of_comments) 
	{
		...
		if(!empty($comment_cats))
		{
			foreach ($comments as $k => $comment) {
				if (!in_category($comment_cats, $comment->comment_post_ID)) {
					unset($comments[$k]);
				}
			}
		}
	...
	if (!(strpos($format,"%gravatar") !== false))
        ...
        if (! $fetched_comments) {
		return "<li>There are no comments yet</li>";
	}
