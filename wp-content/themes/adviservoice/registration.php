...
if ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] ) && $_POST['action'] == 'adduser' ) {
	...
	//validate spam in name
	$firstnamepos = strpos($_POST['first_name'], 'viagra');
	$lastnamepos = strpos($_POST['last_name'], 'viagra');
	...
	else if ($firstnamepos > -1) {
		$error = __('Invalid first name.', 'frontendprofile');
	}
	else if ($lastnamepos > -1) {
		$error = __('Invalid last name.', 'frontendprofile');
	}
	else if ($_POST['postcode']  == "") {
		$error = __('Invalid post code.', 'frontendprofile');
	}
	...
	elseif ( email_exists($userdata['user_email']) )
		...
		$query = "INSERT INTO `wp_sabre_table` SET `user_IP` = '" . $_SERVER['REMOTE_ADDR'] . "', `first_mod` = '" . $stordate . "', `last_mod` = '" . $stordate . "', `user_id`=".$new_user.", `user`='".$userdata['user_login']."', `email`='".$userdata['user_login']."', `status`='to confirm', `md5_id`='".md5($new_user)."'";
		echo $query;
		$wpdb->query($query);
		...
		if( isset($_POST['favorite_notification']) &&  $_POST['favorite_notification'] == '1')
		{
			update_usermeta( $new_user, 'favorite_notification', 1);
		}
		wp_redirect("http://www.adviservoice.com.au/thanksforregistration");
	}
...
<?php get_header(registration); ?>  
	...
	else if(isset( $_SESSION['new_user_id']))
	{
		$user_info = get_userdata($_SESSION['new_user_id']);
	}
	?>
...
<tr>
    ...
    <td width="290"><input id="name" name="cm-name" size="30"  type="text" value="<?php echo $user_info->first_name . ' ' . $user_info->last_name; ?>" /></td>

</tr>

<tr>
	<td><p style="font-size:11px;">Email</p></td>
	<td><input id="nvuyy-nvuyy" name="cm-nvuyy-nvuyy" type="text" size="30"  value="<?php echo $user_info->user_email ?>"  /></td>
</tr>
...
<img class="alignnone no-border marginB_20" title="AdviserVoice newsletters" src="http://www.adviservoice.com.au/wp-content/uploads/stateofplay/AVmail-square.png" alt="AdviserVoice newsletters" width="400" height="400" />
</div>
                                    ...
                                    <li style="padding-top: 6px;"></li><!-- Use one of these social network accounts to login to AdviserVoice. -->
    ...
    <p class="first_name">
              ...
              <p class="form-suburb">
                ...
                <p class="form-dealergroup">
					...
					<img src="<?php bloginfo('template_directory'); ?>/images/global/where-applicable.jpg" width="114" height="23" alt="Where Applicable" />
...
<option <?php if($selval=="Principal") echo "selected" ?> value="Principal">Principal</option>
...
<option <?php if($selval=="Other") echo "selected" ?> value="Other">Other</option>
    ...
    <p class="form-email">
		  ...
		  <p class="form-notification"> 
			<input class="regular-check" name="favorite_notification" id="favorite_notification" type="checkbox" value="1" />
			<label class="extrawidelabel" for="favorite_notification">Check if you want to get an email when there is a new reply for a topic you've commented. </label>
		  </p>
